...
describe("EndPoint /api/v1/mybookmark", () => {
  it("Should be able to get all user's bookmark", async () => {
    const response = await request(app)
      .get("/api/v1/mybookmark")
...
describe("EndPoint /api/v1/bookmark", () => {
  it("Should be able to bookmark by movie id", async () => {
    const response = await request(app)
        .post(`/api/v1/bookmark/${movies[2].id}`)
        .set("Content-Type", "application/json")
        .auth(token, { type: "bearer" });
    ...
    expect(response.body.data.movieTitle).toBe(movies[2].title);
  });
});
